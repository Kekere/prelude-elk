...
class SparqlQueries:
    def __init__(self):
        my_world = World()
        my_world.get_ontology("file:///home/keren/prelude-elk/public_html/vdoowl/onto.owl").load() #path to the owl file is given here
    ...
    def search(self):
        ...
        "SELECT DISTINCT ?v ?val ?priv ?impmethod ?at ?aut ?sce1" \
        "WHERE { " \
        ...
        "{" \
        "SELECT ?v ?priv ?impmethod ?at ?sce" \
        "WHERE {" \
        ...
        for item in resultsList:
            ...
            e = re.sub(r'.*#', "", e)
            response.append({'postcondition' : s, 'privilege' : p, 'impactMethod': o, 'barrier': a, 'mean':e})
            #print(item)
            #return item
